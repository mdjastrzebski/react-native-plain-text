...
| `lineHeight`             | medium | Android only: a `SpannableString` with a span, in place of a plain string. On iOS, two paragraph-style fields.                                                                |
| `letterSpacing`          | light  | One attribute on iOS, one paint write on Android.                                                                                                                             |
| `textDecorationLine`     | light  | One or two attributes on iOS, two paint flags on Android.                                                                                                                     |
| `textShadow*`            | medium | Allocates an `NSShadow` per content build on iOS. The Android side is one `Paint.setShadowLayer` call.                                                                        |
...
style (see
[below](#always-use-the-attributed-string-path-on-ios-iosrnplaintextmm)), so a
prop that maps onto an attribute or a paragraph-style field adds one dictionary
entry or field write and nothing else. That is the argument for expressing a
new iOS text feature as an attribute if it has the choice.
...
| Always use the attributed-string path              | iOS      | mount 5000 labels: interaction 495 → 445 ms, commit flat                |
...
### Always use the attributed-string path on iOS (`ios/RNPlainText.mm`)

`applyContentFromProps` used to have two paths. With none of `lineHeight`,
`letterSpacing`, `textDecorationLine`, `textShadowOffset`, `writingDirection`,
`hyphens="auto"` or `lang` set, it wrote `UILabel`'s plain properties one by one
(`attributedText = nil`, `font`, `textColor`, `textAlignment`, `text`).
Otherwise it built one `NSAttributedString` and wrote `attributedText` once. The
plain path was assumed to be the cheap one, and those props were rated medium
for forcing the other.

The perf suite showed the reverse: 5000 Label-preset items mounted faster with
`lang` set than without, despite a slightly slower commit. An `experiment` A/B
that forced the attributed path with no such prop set confirmed it was the path
and not the language. `UILabel` keeps its own attributed string behind `.text`,
and each of the plain setters plausibly rebuilds or invalidates it, where the
attributed path pays for one build. Not profiled down to the setter.

iPhone 16, physical device, release build, mount 5000 Label-preset items:

|                       | interaction | commit |
| --------------------- | ----------- | ------ |
| Plain path (baseline) | 495 ms      | 122 ms |
| **Attributed path**   | **445 ms**  | 121 ms |

**~10% off interaction**, all of it on the UI thread. The plain path is gone,
along with the `attributedText = nil` reset it needed
([sync-points.md](sync-points.md#set-10--recycled-view-state-ios)). Measurement
is unchanged: it already mirrored the attributed path, which every prop that
forced it went through.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **iOS mount path**                                                                                                                                     | Now the largest known iOS target: ~62% of interaction                        | **Trigger has fired.** The device split at Large puts ~102 ms of a 161 ms interaction on the UI thread against 59 ms of commit, so the remaining headroom is in mounting, not measuring ([above](#ios-iphone-16-physical-device)). Every mount-path fix before [always using the attributed-string path](#always-use-the-attributed-string-path-on-ios-iosrnplaintextmm) was Android-only, and nothing checks whether `applyContentFromProps` rebuilds the attributed string more than once per transaction, the exact problem prop batching solved on Android | Ready now. [Always using the attributed-string path](#always-use-the-attributed-string-path-on-ios-iosrnplaintextmm) took ~10%; next, instrument `updateProps:` for the `lineBreakMode`/`lineBreakStrategy`/`numberOfLines` writes that follow the `attributedText` write |

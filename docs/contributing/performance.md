...
| `hyphenationFactor`      | medium | iOS only. Forces the iOS attributed-string path (no standalone `UILabel` property for it). No-op on Android.                                                                  |
...
`textDecorationLine`, `textShadowOffset`, `writingDirection` and
`hyphenationFactor` are all unset (at their defaults). Any one of them puts the
node on the `NSAttributedString` path for good. A further prop that needs an
attributed-string attribute is therefore free on top of the first, and that is
the argument for expressing a new iOS text feature as one if it has the
choice.

...
Nothing is heavy today. The constrained-only `adjustsFontSizeToFit`
(docs/contributing/adjusts-font-size-to-fit.md) stays out of heavy by not
running a general shrink search: single line only, so Android estimates the
fit from one `Paint.measureText` call instead of bisecting a `StaticLayout`,
and iOS hands the search to UIKit's own `adjustsFontSizeToFitWidth` instead of
building one.

| Prop                     | Cost   | Why                                                                                                                                                                                                                                                                                                                          |
| ------------------------ | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `adjustsFontSizeToFit`   | medium | iOS: native `UILabel` machinery, paid only with `numberOfLines={1}`. Android: a scratch `TextPaint` copy plus one `Paint.measureText` call once the view is eligible (opted in, single line, a definite width) at all, more `measureText` calls (up to `MAX_SHRINK_ITERATIONS`) only while the text still overflows the box. |

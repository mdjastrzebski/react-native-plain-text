...
| Prop                     | Cost   | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fontFamily`             | medium | First resolution hits the system font database on iOS and the asset lookup on Android. Cached after, per family and size. On Android, also pushes measurement onto the slower path below, same as `fontWeight`/`fontStyle`.                                                                                                                                                                                                                                       |
| `fontWeight`/`fontStyle` | medium | Android only: setting either (even to a value that resolves to the font's own default, e.g. an explicit `fontStyle="normal"`) matches RN's `<Text>` by pushing `Layout.getDesiredWidth` onto Android's unhinted glyph-shaping path instead of the cheaper hinted default (`PlainTextView.applyTypeface()`, [perf-experiments.md](perf-experiments.md)). Measured ~2.5% extra mount cost. No iOS cost beyond the shared font resolution `fontFamily` already pays. |
...
Not an optimization, the opposite: a correctness fix with a real, accepted
cost. `applyTypeface()` now sets `paint.isSubpixelText`/`isLinearText` to
match RN's own `CustomStyleSpan`, whenever `fontFamily`/`fontWeight`/
`fontStyle` is set at all (the exact condition RN's `TextLayoutManager` uses).
Full history and root cause in [perf-experiments.md](perf-experiments.md);
this entry is the cost half.

Real-device measurement (avg of 5, release build): mounting 1000
OpenSans-styled nodes moved `interaction` from 610ms to 625ms, ~2.5%. The cost
is Android's unhinted glyph-shaping path in `Layout.getDesiredWidth`
(`PlainTextViewManager.measure()`), not the paint writes themselves, which
are a plain field assignment each. Two cheaper alternatives were considered
and set aside rather than built, both in perf-experiments.md's "Current
state": confirming which of the two flags actually causes the cost (they've
only ever been toggled together), and whether the off-screen measuring
view's copy of the cost (which never draws, so has no visual reason to pay
for unhinted shaping) could be avoided separately from the mounted view's.
Neither has been measured, so neither is claimed as a win here.

Accepted without chasing further: the correctness win (pixel-exact width and
glyph-position parity with RN's `<Text>`) was judged worth ~15ms/1000 nodes
without more optimization for now. Revisit if a profile shows this dominating
a real screen's mount cost, or if #1 above narrows where to look.
...
| **Isolate `isLinearText` vs `isSubpixelText` cost** (`PlainTextView.kt`, Android)                    | Could halve the ~2.5% mount cost of [matching RN's `CustomStyleSpan`](#match-rns-customstylespan-paint-flags-for-custom-styled-text-plaintextviewkt-android-only) if one flag dominates | The two have only ever been toggled together, mirroring RN's own span; no on-device split has been run                                                                                                                                                                                                                                                                                                                                                                                                      | A profile of custom-font-heavy mount cost, or someone chasing the ~2.5% number down                                                    |
| **Scope the `CustomStyleSpan` paint flags to measurement only** (`PlainTextViewManager.kt`, Android) | Avoids paying draw-time unhinted-shaping cost on every redraw/relayout of a mounted custom-font node, not just once at mount                                                            | Correctness risk: the mounted view's own internal `Layout` (draw) and the ad-hoc `getDesiredWidth` call (Yoga's width) would use different paint states, reopening a small gap between drawn glyph positions and the box Yoga sized for them                                                                                                                                                                                                                                                                | A scrolling/steady-state benchmark (not just cold mount) shows this cost recurring, and the drawn-vs-measured gap is judged acceptable |

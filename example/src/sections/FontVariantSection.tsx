...
export function FontVariantSection({ showText }: { showText: boolean }) {
  return (
    <Section title="Font Variant" footer={FONT_VARIANT_FOOTER}>
      <TextItem
        testID={featureCaptureTestID('font-variant', 'system-default')}
        label="default"
        showText={showText}
      ...
      {TABULAR_FIGURE_ROWS.map((digits) => (
        <TextItem
          key={`tabular-${digits}`}
          testID={featureCaptureTestID('font-variant-tabular-nums', digits)}
          label="tabular-nums"
          showText={showText}
      ...
      {TABULAR_FIGURE_ROWS.map((digits) => (
        <TextItem
          ...
          testID={featureCaptureTestID('font-variant-proportional-nums', digits)}
      ...
      <TextItem
        testID={featureCaptureTestID('font-variant', 'feature-font-default')}
        label="default"
        showText={showText}
      ...
      {FONT_VARIANTS.map(({ label, fontVariant }) => (
        <TextItem
          key={label}
          testID={featureCaptureTestID('font-variant', label)}
          label={label}
          showText={showText}

...
export function featureCaptureTestID(section: string, specimen: string | number) {
  const slug = String(specimen)
    .toLowerCase()
    .replace(/^-/, 'negative-')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  return `vrt-capture-features-${section}-${slug}`;
}
...
export function Section({
  ...
  spacedRows?: boolean;
  // Stable VRT names for direct TextItem/CompareBox children, in render order.
  captureNames?: readonly string[];
  children: ReactNode;
}) {
  const vrtTestID = useContext(VrtSpecimenContext);
  const capturedChildren =
    captureNames == null
      ? children
      : Children.toArray(children).map((child, index) =>
          isValidElement<{ testID?: string }>(child) && captureNames[index] != null
            ? cloneElement(child, { testID: `vrt-capture-use-cases-${captureNames[index]}` })
            : child
        );
  ...
  return (
    <View testID={testID} style={styles.rowContainer}>
      ...
      <View style={styles.specimen}>
        <View
          testID={testID == null ? undefined : `${testID}-text`}
        ...
        {showText && (
          <View testID={testID == null ? undefined : `${testID}-rn-text`} style={styles.overlay}>
            <Text
